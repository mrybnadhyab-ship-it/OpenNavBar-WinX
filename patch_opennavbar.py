...
if "WINX_CLOCK_LAYOUT_PATCH" not in code:
    ...
    if not icon_path.exists():
        raise RuntimeError("Adobe_20230903_191353.png not found in repository root")
    adobe_icon_base64 = base64.b64encode(icon_path.read_bytes()).decode("ascii")
    ...
    clock_layout_patch = r'''
        ...
        // WINX_MICROSOFT_STORE_BUTTON_PATCH
        val winXMicrosoftStoreButton = android.widget.FrameLayout(this).apply {
            isClickable = true
            isFocusable = true
            isLongClickable = false

            val storeIcon = android.widget.ImageView(this@NavigationOverlayService).apply {
                try {
                    val iconBase64 = "__ADOBE_ICON_BASE64__"
                    val iconBytes = android.util.Base64.decode(
                        iconBase64,
                        android.util.Base64.DEFAULT
                    )
                    setImageBitmap(
        ...
        // shouldSwap: Recent | Clock | SPACE | Gmail | Microsoft | Home | Back
        // normal:    Back | Home | Microsoft | Gmail | SPACE | Clock | Recent
        if (shouldSwap) {
            container.addView(winXSpacer, 2)
            container.addView(winXGmailButton, 3)
            container.addView(winXMicrosoftStoreButton, 4)
        } else {
            container.addView(winXMicrosoftStoreButton, 2)
            container.addView(winXGmailButton, 3)
...
'''
    ...
    clock_layout_patch = clock_layout_patch.replace("__ADOBE_ICON_BASE64__", adobe_icon_base64)
...
print("Microsoft Store: 20dp icon / Back > Home > Microsoft > Gmail > SPACE > Clock > Recent")
print("Xiaomi Community: removed")
print("Microsoft Store: uses Adobe_20230903_191353.png / 16dp")
