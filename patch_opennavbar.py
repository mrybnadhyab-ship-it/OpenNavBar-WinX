...
# ============================================================
# 8. FULLSCREEN LOCK
#    Keep OpenNavBar visible in fullscreen.
#    Win-X hiding remains unchanged.
#    Other original Swipe/Reveal behavior remains untouched.
# ============================================================

if "WINX_FULLSCREEN_LOCK_PATCH" not in code:
    fullscreen_expr = 'prefs.getBoolean("hide_on_fullscreen", true)'

    if fullscreen_expr in code:
        code = code.replace(
            fullscreen_expr,
            'false /* WINX_FULLSCREEN_LOCK_PATCH */',
        )
        print("Fullscreen: LOCKED VISIBLE (hide_on_fullscreen disabled)")
    else:
        # Do not fail the build if the upstream source uses a different
        # fullscreen implementation. The rest of the patch remains usable.
        print("Warning: hide_on_fullscreen preference not found; fullscreen lock not applied")
...
print("Fullscreen: OpenNavBar stays visible")
