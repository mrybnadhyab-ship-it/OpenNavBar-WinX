...
if "WINX_CLOCK_LAYOUT_PATCH" not in code:
    ...
    clock_layout_patch = r'''
        ...
        // WINX_XIAOMI_STORE_BUTTON_PATCH

        val winXXiaomiButton = android.widget.FrameLayout(this).apply {
            isClickable = true
            isFocusable = true
            isLongClickable = false

            val icon = android.widget.ImageView(this@NavigationOverlayService).apply {
                try {
                    setImageDrawable(
                        packageManager.getApplicationIcon("com.mi.global.bbs")
                    )
                } catch (_: Exception) {
            ...
            setOnClickListener {
                try {
                    val launchIntent = packageManager.getLaunchIntentForPackage("com.mi.global.bbs")
        ...
        val winXMicrosoftStoreButton = android.widget.FrameLayout(this).apply {
            isClickable = true
            isFocusable = true
            isLongClickable = false

            val icon = android.widget.ImageView(this@NavigationOverlayService).apply {
                // Windows 11 Microsoft Store-style blue shopping bag, drawn locally.
                val size = dpToPx(16)
                val drawable = object : android.graphics.drawable.Drawable() {
                    ...
                    override fun draw(canvas: android.graphics.Canvas) {
                        ...
                        paint.color = android.graphics.Color.rgb(0, 120, 212)
                        val bag = android.graphics.Path().apply {
                            moveTo(w * 0.16f, h * 0.30f)
                            quadTo(w * 0.16f, h * 0.18f, w * 0.28f, h * 0.18f)
                            lineTo(w * 0.72f, h * 0.18f)
                            quadTo(w * 0.84f, h * 0.18f, w * 0.84f, h * 0.30f)
                            lineTo(w * 0.92f, h * 0.88f)
                            quadTo(w * 0.92f, h * 0.96f, w * 0.84f, h * 0.96f)
                            lineTo(w * 0.16f, h * 0.96f)
                            quadTo(w * 0.08f, h * 0.96f, w * 0.08f, h * 0.88f)
                            close()
                        }
                        ...
                        val handle = android.graphics.Path().apply {
                            ...
                            cubicTo(w * 0.34f, h * 0.07f, w * 0.66f, h * 0.07f, w * 0.66f, h * 0.30f)
                        }
                        canvas.drawPath(handle, paint)
                        paint.style = android.graphics.Paint.Style.FILL
                        paint.color = android.graphics.Color.WHITE
                        canvas.drawCircle(w * 0.40f, h * 0.58f, h * 0.045f, paint)
                        canvas.drawCircle(w * 0.60f, h * 0.58f, h * 0.045f, paint)
                        val win = android.graphics.Path().apply {
                            moveTo(w * 0.41f, h * 0.69f)
                            lineTo(w * 0.50f, h * 0.69f)
                            lineTo(w * 0.50f, h * 0.82f)
                            lineTo(w * 0.41f, h * 0.82f)
                            close()
                            moveTo(w * 0.53f, h * 0.69f)
                            lineTo(w * 0.62f, h * 0.69f)
            ...
            setOnClickListener {
                try {
                    val intent = android.content.Intent(
                        ...
                        android.net.Uri.parse("https://apps.microsoft.com/")
                    ).apply {
                        addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK)
                    }
        ...
        // WINX_XIAOMI_STORE_BUTTON_PATCH_END

        // WINX_GMAIL_POSITION_PATCH
        // shouldSwap: Recent | Clock | SPACE | Gmail | Store | Xiaomi | Home | Back
        // normal:    Back | Home | Xiaomi | Store | Gmail | SPACE | Clock | Recent
        if (shouldSwap) {
            container.addView(winXGmailButton, 3)
            container.addView(winXMicrosoftStoreButton, 4)
            container.addView(winXXiaomiButton, 5)
        } else {
            container.addView(winXXiaomiButton, 2)
            container.addView(winXMicrosoftStoreButton, 3)
            container.addView(winXGmailButton, 4)
        }
...
print("Xiaomi Community: 16dp icon / beside Gmail on clock side")
print("Microsoft Store: Windows 11-style blue bag / 16dp")
