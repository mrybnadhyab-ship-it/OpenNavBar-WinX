...
if "WINX_STABLE_PATCH" not in code:
    ...
    // WINX_GMAIL_ICON_PATCH

    private fun createWinXGmailButton(): android.widget.ImageView? {
        val gmailPackage = "com.google.android.gm"

        return try {
            val pm = packageManager
            val appInfo = pm.getApplicationInfo(gmailPackage, 0)
            ...
            android.widget.ImageView(this).apply {
                ...
                setOnClickListener {
                    try {
                        val launchIntent =
                            pm.getLaunchIntentForPackage(gmailPackage)
            ...
            // Gmail not installed: no empty button is added.
            null
        }
    }

    // WINX_GMAIL_ICON_PATCH_END
...
if "WINX_CLOCK_LAYOUT_PATCH" not in code:
    ...
    clock_layout_patch = r'''
        ...
        // WINX_GMAIL_LAYOUT_PATCH

        createWinXGmailButton()?.let { gmailButton ->
            val gmailParams = if (isVerticalBar) {
                LinearLayout.LayoutParams(
            ...
            // Gmail pinned app icon; original navigation remains untouched.
            val gmailIndex = if (shouldSwap) 1 else 2
            val safeIndex = gmailIndex.coerceAtMost(container.childCount)
            container.addView(gmailButton, safeIndex, gmailParams)
        }

        // WINX_GMAIL_LAYOUT_PATCH_END
